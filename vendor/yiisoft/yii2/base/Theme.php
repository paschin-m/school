 ...
 *
 * When [[View]] renders a views file, it will check the [[View::theme|active theme]]
 * to see if there is a themed version of the views file exists. If so, the themed version will be rendered instead.
 *
 * A theme is a directory consisting of views files which are meant to replace their non-themed counterparts.
 *
 * Theme uses [[pathMap]] to achieve the views file replacement:
 *
 * 1. It first looks for a key in [[pathMap]] that is a substring of the given views file path;
 * 2. If such a key exists, the corresponding value will be used to replace the corresponding part
 *    in the views file path;
 * 3. It will then check if the updated views file exists or not. If so, that file will be used
 *    to replace the original views file.
 * 4. If Step 2 or 3 fails, the original views file will be used.
 *
 * For example, if [[pathMap]] is `['@app/views' => '@app/themes/basic']`,
 * then the themed version for a views file `@app/views/site/index.php` will be
 ...
 *
 * To use a theme, you should configure the [[View::theme|theme]] property of the "views" application
...
{
    /**
     * @var array the mapping between views directories and their corresponding themed versions.
     * This property is used by [[applyTo()]] when a views is trying to apply the theme.
